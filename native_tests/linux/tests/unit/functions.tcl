# This file is ported from valkey-io/valkey (tests/unit/functions.tcl, v9.1.0),
# licensed under the 3-Clause BSD License (see the Valkey repository LICENSE).
#
# Local modifications for jedis-mock:
#  - the server.* API is renamed to redis.*; jedis-mock models the redis.* API;
#  - tests for commands/features jedis-mock does not yet model are disabled
#    inline with `if 0 { ... }`, each tagged with the specific missing
#    feature;
#  - a few assertions are loosened from upstream to match current fidelity
#    gaps rather than being disabled outright; each is noted inline where it
#    occurs.
#
...
start_server {tags {"scripting"}} {
    ...
    # FUNCTION DUMP/RESTORE is not implemented.
    if 0 {
    ...
    # COMMAND (and so COMMAND GETKEYS) is not implemented.
    if 0 {
    ...
    test {FUNCTION - test function kill} {
        ...
        # loosened from upstream's {ERR Script killed by user with FUNCTION KILL*}:
        # real redis always reports "with SCRIPT KILL", regardless
        # of whether a SCRIPT or a FUNCTION was actually killed.
        assert_error {ERR Script killed by user*} {$rd read}
        $rd close
    }
    ...
    # INFO's used_memory_vm_functions stat is not modeled.
    if 0 {
...
start_server {tags {"scripting"}} {
    ...
    test {LIBRARIES - register function inside a function} {
        ...
        set _ $e
    # relaxed error message assertion due to luaj and lua's C VM discrepancies
    ...
    # Depends on FUNCTION DUMP/RESTORE and maxmemory/OOM enforcement, neither of which is implemented.
    if 0 {
    ...
    # redis.register_function's flags argument is a stub (always rejected, see Function.java) and maxmemory/OOM enforcement is not implemented.
    if 0 {
...
start_server {tags {"scripting"}} {
    ...
    # redis.register_function's flags argument is a stub (always rejected, see Function.java), so no-writes enforcement can't be tested.
    if 0 {
    ...
    # maxmemory/OOM enforcement is not implemented.
    if 0 {
    ...
    # Depends on maxmemory/OOM enforcement, the flags stub, and FCALL_RO, none of which is implemented.
    if 0 {
    ...
    # Depends on REPLICAOF/replication (not implemented) and the flags stub.
    if 0 {
    ...
    # redis.REDIS_VERSION / redis.REDIS_VERSION_NUM are not implemented.
    if 0 {
